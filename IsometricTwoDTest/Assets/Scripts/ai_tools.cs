    ...
    public class ai_tools
    {
        ...
        public void build_building(Tile tile, building_type type, int civilization)
        {
            ...
            if (!tile.has_building() && tile.is_walkable() && match_manager.choose_player(civilization).gold >= type.buildCost)
            {
                ...
                {
                    import_manager.run_function_all("preview_object", "build_building", new string[3] { type.get_building_of_civilization(civilization).name, tile.get_grid()[0].ToString(), tile.get_grid()[1].ToString() });
        ...
        public void build_units(Tile tile, unit_type type, int civilization)
        {
            ...
            if (!tile.is_occupied() && tile.is_walkable() && tile.is_in_city() && match_manager.choose_player(civilization).food >= type.food)
            {
                ...
                import_manager.run_function_all("unit_manager", "add_unit", new string[4] { tile.get_grid()[0].ToString(), tile.get_grid()[1].ToString(), type.get_unit_of_civilization(civilization).gameObject.name, match_manager.choose_player(civilization).civilization.ToString() });
            }
        }
    }
}
